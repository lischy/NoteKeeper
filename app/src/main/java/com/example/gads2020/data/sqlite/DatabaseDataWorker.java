...
public class DatabaseDataWorker {
    ...
    private void insertCourse(String courseId, String title) {
        ...
        values.put(myNoteContract.courseInfoEntry.COLUMN_COURSE_ID, courseId);
        values.put(myNoteContract.courseInfoEntry.COLUMN_COURSE_TITLE, title);

        long newRowId = mDb.insert(myNoteContract.courseInfoEntry.TABLE_NAME, null, values);
    }

     private void insertNote(String courseId, String title, String text) {
        ContentValues values = new ContentValues();
        values.put(myNoteContract.noteInfoEntry.COLUMN_COURSE_ID, courseId);
        values.put(myNoteContract.noteInfoEntry.COLUMN_NOTE_TITLE, title);
        values.put(myNoteContract.noteInfoEntry.COLUMN_NOTE_TEXT, text);

        long newRowId = mDb.insert(myNoteContract.noteInfoEntry.TABLE_NAME, null, values);
    }
