...
//            ourInstance.initializeExampleNotes();
    ...
    public int createNewNote(int id) {
        NoteInfo note = new NoteInfo(id, null, null, null);
        mNotes.add(note);
    ...
    //Using th db to load data

    public static void loadDataFromDatabase(NoteKeeperOpenHelper myHelper){
        ...
        String courseSortOrder = courseInfoEntry.COLUMN_COURSE_TITLE ;
    ...
    private static void loadNotesFromDatabase(Cursor noteCursor) {
        int noteTitleColumnPosition = noteCursor.getColumnIndexOrThrow(noteInfoEntry.COLUMN_NOTE_TITLE);
        int noteTextColumnPosition = noteCursor.getColumnIndexOrThrow(noteInfoEntry.COLUMN_NOTE_TEXT);
        int courseIdColumnPosition = noteCursor.getColumnIndexOrThrow(noteInfoEntry.COLUMN_COURSE_ID);
        int idColumnPosition = noteCursor.getColumnIndexOrThrow(noteInfoEntry._ID);

        DataManager manager = getInstance();
        manager.mNotes.clear();
        ...
        while(noteCursor.moveToNext()){
            ...
            String courseIdDB = noteCursor.getString(courseIdColumnPosition);
            int id = noteCursor.getInt(idColumnPosition);

            CourseInfo noteCourse = manager.getCourse(courseIdDB);
            NoteInfo note = new NoteInfo(id,noteCourse,noteTitleDB,noteTextDB);
            manager.mNotes.add(note);
        }
        noteCursor.close();
    }

    private static void loadCoursesFromDatabase(Cursor courseCursor) {
        int courseIdColumnPosition = courseCursor.getColumnIndexOrThrow(courseInfoEntry.COLUMN_COURSE_ID);
        int courseTitleColumnPosition = courseCursor.getColumnIndexOrThrow(courseInfoEntry.COLUMN_COURSE_TITLE);

        DataManager dm = getInstance();
        dm.mCourses.clear();

        while (courseCursor.moveToNext()){
            String courseIdDB = courseCursor.getString(courseIdColumnPosition);
            String courseTitleDB = courseCursor.getString(courseTitleColumnPosition);
            CourseInfo course = new CourseInfo(courseIdDB,courseTitleDB,null);
    ...
    public void initializeExampleNotes(int id ) {
        final DataManager dm = getInstance();

        CourseInfo course = dm.getCourse("android_intents");
        course.getModule("android_intents_m01").setmIsComplete(true);
        course.getModule("android_intents_m02").setmIsComplete(true);
        course.getModule("android_intents_m03").setmIsComplete(true);
        mNotes.add(new NoteInfo(id, course, "Dynamic intent resolution",
                "Wow, intents allow components to be resolved at runtime"));
        mNotes.add(new NoteInfo(id, course, "Delegating intents",
                "PendingIntents are powerful; they delegate much more than just a component invocation"));

        course = dm.getCourse("android_async");
        course.getModule("android_async_m01").setmIsComplete(true);
        course.getModule("android_async_m02").setmIsComplete(true);
        mNotes.add(new NoteInfo(id, course, "Service default threads",
                "Did you know that by default an Android Service will tie up the UI thread?"));
        mNotes.add(new NoteInfo(id, course, "Long running operations",
                "Foreground Services can be tied to a notification icon"));

        course = dm.getCourse("java_lang");
        course.getModule("java_lang_m01").setmIsComplete(true);
        course.getModule("java_lang_m02").setmIsComplete(true);
        course.getModule("java_lang_m03").setmIsComplete(true);
        course.getModule("java_lang_m04").setmIsComplete(true);
        course.getModule("java_lang_m05").setmIsComplete(true);
        course.getModule("java_lang_m06").setmIsComplete(true);
        course.getModule("java_lang_m07").setmIsComplete(true);
        mNotes.add(new NoteInfo(id, course, "Parameters",
                "Leverage variable-length parameter lists"));
        mNotes.add(new NoteInfo(id, course, "Anonymous classes",
                "Anonymous classes simplify implementing one-use types"));

        course = dm.getCourse("java_core");
        course.getModule("java_core_m01").setmIsComplete(true);
        course.getModule("java_core_m02").setmIsComplete(true);
        course.getModule("java_core_m03").setmIsComplete(true);
        mNotes.add(new NoteInfo(id, course, "Compiler options",
                "The -jar option isn't compatible with with the -cp option"));
        mNotes.add(new NoteInfo(id, course, "Serialization",
